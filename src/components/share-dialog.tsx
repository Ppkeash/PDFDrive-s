...
import { Check, Clock, Crown, Link2, Share2, X } from "lucide-react";
...
export function ShareDialog({
  ...
  shares: ShareRow[];
  ownerEmail: string;
  /** Token vigente del link único, o null si nunca se generó / se revocó. */
  ...
  function makeLink() {
    ...
    startTransition(async () => {
      const res = await generateInviteLink(documentId);
  ...
  function revokeLink() {
    ...
    setNotice(null);
    startTransition(async () => {
      const res = await revokeInviteLink(documentId);
      if (res.error) return setError(res.error);
      setToken(null);
      router.refresh();
    });
          ...
          >
            <div className="flex shrink-0 items-center justify-between border-b border-line px-5 py-4">
              <h2 id="share-title" className="font-display text-lg font-semibold">
                {compact ? "Gestionar accesos" : "Compartir documento"}
              </h2>
              <button
                onClick={() => setOpen(false)}
                aria-label="Cerrar"
            ...
            <div className="flex min-h-0 flex-1 flex-col gap-5 overflow-y-auto p-5">
              {/* Compartir: solo invitar. Gestionar ya lo hace desde la lista de abajo. */}
              {!compact && (
              ...
              {/* Gestionar: lista con rol y quitar acceso. Compartir no la necesita. */}
              {compact && (
